...
hash[:response][:results].each do |aritcle|
  article[:view] = 0
end

def read_article 
  article_count = hash[:response][:results].count
  random_num = random(1..article_count)
  hash[:response][:results][random_num][:views] += 1
end
